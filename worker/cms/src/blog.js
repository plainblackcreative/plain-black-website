...
// It's regenerated by `npm run sync-tracking` (pre-push hook) and carries
// title + ogTitle + lastModified for every blog post. Reading it is ONE
// GitHub call regardless of how many posts exist; reading each post file
// individually blows past the Workers subrequest limit at ~50 posts.
//
// Trade-off: a post created via the CMS won't show up here until the
// manifest is regenerated on the next `git push`. For now that's fine; if
// we hit the case often we can have the CMS append a placeholder entry to
...
export async function listPosts(env) {
  ...
  return blogPages
    .map(p => ({
      slug: p.slug,
      // Prefer the full <title> tag (with ` | PlainBlack` suffix stripped) over
      // ogTitle, because the manifest builder currently truncates ogTitle at
      // apostrophes ("We're..." -> "We"). Fix that script separately.
      title: ((p.title || '').replace(/\s*\|\s*PlainBlack\s*$/, '') || p.ogTitle || p.slug),
